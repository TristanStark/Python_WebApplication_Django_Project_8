...
def feed(request):
    ...
    following_tickets = Ticket.objects.filter(user__in=following_users).order_by(
        "-time_created"
    )
    following_reviews = Review.objects.filter(user__in=following_users).order_by(
        "-time_created"
    )
    ...
    posts = sorted(
        list(own_tickets)
        + list(own_reviews)
        + list(following_tickets)
        + list(following_reviews),
